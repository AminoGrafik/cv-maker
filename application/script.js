document.addEventListener('DOMContentLoaded', function() {
    ...
    function addCard(type) {
        ...
        let content = '';
        let uniqueId = Date.now(); // Simple way to get unique IDs for Trix
        ...
        switch(type) {
            case 'workExperience':
                content = `
                    ...
                    <div class="form-card-body">
                        ...
                        <div class="form-card-grid-2">
                            <div class="form-group"><label>Start Date</label><input type="text" class="work-start-date"></div>
                            <div class="form-group"><label>End Date</label><input type="text" class="work-end-date"></div>
                        </div>
                        <div class="form-group"><label>Description</label><input class="work-description" id="work-desc-${uniqueId}" type="hidden"><trix-editor input="work-desc-${uniqueId}"></trix-editor></div>
                    </div>`;
                break;
            case 'projects':
                content = `
                    ...
                    <div class="form-card-body">
                        ...
                        <div class="form-group"><label>Description</label><input class="project-description" id="proj-desc-${uniqueId}" type="hidden"><trix-editor input="proj-desc-${uniqueId}"></trix-editor></div>
                    </div>`;
                break;
            case 'education':
                 content = `
            ...
            case 'languages':
                content = `
                    <div class="form-card-header">
                        <span class="icon"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m5 8 6 6"/><path d="m4 14 6-6 2-3"/><path d="M2 5h12"/><path d="M7 2h1"/><path d="m22 22-5-10-5 10"/><path d="M14 18h6"/></svg></span>
                        <div class="title">Language</div>
                        <span class="chevron"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"></polyline></svg></span>
                    </div>
                    <div class="form-card-body form-card-grid-2">
                        <div class="form-group"><label>Language</label><input type="text" class="card-title-input lang-name"></div>
                        <div class="form-group"><label>Proficiency</label><input type="text" class="lang-prof"></div>
                    </div>`;
                break;
        }
    ...
    function collectData() {
        const getCardData = (containerId, fields) => {
            return Array.from(document.querySelectorAll(`#${containerId} .form-card`)).map(card => {
                const entry = {};
                for (const field in fields) {
                    entry[field] = card.querySelector(fields[field])?.value || '';
                }
                return entry;
            });
        };
        return {
            personal: {
                fullName: document.getElementById('fullName').value, jobTitle: document.getElementById('jobTitle').value, email: document.getElementById('email').value, phone: document.getElementById('phone').value, location: document.getElementById('location').value,
            },
            summary: document.getElementById('summary').value,
            skills: document.getElementById('skills').value,
            experience: getCardData('workExperience', { title: '.work-title', company: '.work-company', startDate: '.work-start-date', endDate: '.work-end-date', description: '.work-description' }),
    ...
    function generateCV() {
        ...
        const section = (title, content) => content ? `<div class="cv-section"><div class="cv-section-title">${title}</div>${content}</div>` : '';
        ...
        const summaryHTML = section('Short Bio', data.summary);
        
        const experienceHTML = section('Experience', data.experience.map(exp => `
            <div class="cv-item">
        ...
        const languagesHTML = section('Languages', `<div class="skills-grid">${data.languages.map(lang => `<div class="skill-item">${lang.name} (${lang.proficiency})</div>`).join('')}</div>`);

        preview.innerHTML = [headerHTML, summaryHTML, experienceHTML, projectsHTML, educationHTML, skillsHTML, languagesHTML].join('');
    }
