document.addEventListener('DOMContentLoaded', function() {
    ...
    const collectData = () => ({
        ...
        projects: Array.from(document.querySelectorAll('#projects .dynamic-section')).map(s => ({ name: s.querySelector('.proj-name').value, link: s.querySelector('.proj-link').value, description: s.querySelector('.proj-description').value, })),
        education: Array.from(document.querySelectorAll('#education .dynamic-section')).map(s => ({ degree: s.querySelector('.edu-degree').value, institution: s.querySelector('.edu-institution').value, date: s.querySelector('.edu-date').value, })),
        awards: Array.from(document.querySelectorAll('#awards .dynamic-section')).map(s => ({ name: s.querySelector('.award-name').value, date: s.querySelector('.award-date').value, })),
        publications: Array.from(document.querySelectorAll('#publications .dynamic-section')).map(s => ({ title: s.querySelector('.pub-title').value, journal: s.querySelector('.pub-journal').value, date: s.querySelector('.pub-date').value, })),
        volunteer: Array.from(document.querySelectorAll('#volunteer .dynamic-section')).map(s => ({ role: s.querySelector('.vol-role').value, org: s.querySelector('.vol-org').value, date: s.querySelector('.vol-date').value, description: s.querySelector('.vol-description').value, })),
        certifications: Array.from(document.querySelectorAll('#certifications .dynamic-section')).map(s => ({ name: s.querySelector('.cert-name').value, org: s.querySelector('.cert-org').value, date: s.querySelector('.cert-date').value, })),
        languages: Array.from(document.querySelectorAll('#languages .dynamic-section')).map(s => ({ name: s.querySelector('.lang-name').value, proficiency: s.querySelector('.lang-prof').value, })),
    });
    ...
    const generateProjects = projs => generateSection('Projects', projs.map(proj => {
        ...
        if (proj.link) {
            ...
            projectLink = `<a href="${fullUrl}" target="_blank" rel="noopener noreferrer" class="cv-item-date">${trimmedLink}</a>`;
        }
    ...
    const addSection = (type) => {
        ...
        switch(type) {
            case 'workExperience': count = ++workExperienceCount; headerText = `Experience ${count}`; fields = `<input type="text" placeholder="Job Title" class="work-title"><input type="text" placeholder="Company" class="work-company"><input type="text" placeholder="Date" class="work-date">${createTrixEditor(`work-desc-${count}`, 'work-description')}`; break;
            case 'projects': count = ++projectsCount; headerText = `Project ${count}`; fields = `<input type="text" placeholder="Project Name" class="proj-name"><input type="text" placeholder="Project Link (e.g., myproject.com)" class="proj-link">${createTrixEditor(`proj-desc-${count}`, 'proj-description')}`; break;
            case 'education': count = ++educationCount; headerText = `Education ${count}`; fields = `<input type="text" placeholder="Degree" class="edu-degree"><input type="text" placeholder="Institution" class="edu-institution"><input type="text" placeholder="Date" class="edu-date">`; break;
            case 'awards': count = ++awardsCount; headerText = `Award ${count}`; fields = `<input type="text" placeholder="Award Name (e.g., Dean's List)" class="award-name"><input type="text" placeholder="Date" class="award-date">`; break;
            case 'publications': count = ++publicationsCount; headerText = `Publication ${count}`; fields = `<input type="text" placeholder="Title of Publication" class="pub-title"><input type="text" placeholder="Journal or Conference" class="pub-journal"><input type="text" placeholder="Date" class="pub-date">`; break;
            case 'volunteer': count = ++volunteerCount; headerText = `Volunteer Role ${count}`; fields = `<input type="text" placeholder="Role (e.g., Event Coordinator)" class="vol-role"><input type="text" placeholder="Organization" class="vol-org"><input type="text" placeholder="Date" class="vol-date">${createTrixEditor(`vol-desc-${count}`, 'vol-description')}`; break;
            case 'certifications': count = ++certificationCount; headerText = `Certification ${count}`; fields = `<input type="text" placeholder="Certification Name" class="cert-name"><input type="text" placeholder="Issuing Organization" class="cert-org"><input type="text" placeholder="Date" class="cert-date">`; break;
            case 'languages': count = ++languagesCount; headerText = `Language ${count}`; fields = `<input type="text" placeholder="Language" class="lang-name"><input type="text" placeholder="Proficiency (e.g., Native)" class="lang-prof">`; break;
        }
        ...
        div.querySelectorAll('input:not([type=hidden]), trix-editor').forEach(el => { el.addEventListener('input', debouncedGenerateCV); });
        generateCV();
    };
    ...
    document.querySelectorAll('.editor-panel input, .editor-panel textarea, .editor-panel trix-editor').forEach(el => { el.addEventListener('input', debouncedGenerateCV); });
